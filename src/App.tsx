...
export default function App() {
  ...
  useEffect(() => {
    if (currentTab === 'battle' && currentMatch) {
      document.title = `${currentMatch.creator1.name} vs ${currentMatch.creator2.name} - 1v1Vote Live Arena`;
      const metaDesc = document.querySelector('meta[name="description"]');
      if (metaDesc) {
        metaDesc.setAttribute(
          'content',
          `Vote in the live 1v1 battle between ${currentMatch.creator1.name} and ${currentMatch.creator2.name}. Track live vote swings, unlimited likes, and stock-style charts!`
        );
      }
    } else {
      document.title = '1v1Vote - Live Creator Battles & Matchup Voting';
    }
  ...
  const handleLogin = (name: string, email: string) => {
    const profile = MatchStore.login(name, email);
    setUser(profile);
