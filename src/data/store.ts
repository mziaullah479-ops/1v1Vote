...
const LIKES_STORAGE_KEY = 'vs_battle_user_likes_v1';
...
export class MatchStore {
  ...
  private static userLikes: Record<string, boolean> = {};
  ...
  private static async backendRequest<T>(path: string, init: RequestInit = {}): Promise<T> {
    ...
    if (!response.ok) {
      const error = new Error(data.error || `Backend request failed (${response.status})`) as Error & { status?: number; code?: string };
  ...
  private static applyBackendMatch(match: Match) {
    const index = this.matches.findIndex((item) => item.id === match.id);
    if (index === -1) this.matches = [match, ...this.matches];
    else this.matches[index] = match;
  }

  private static applyBackendComment(comment: Comment) {
    const list = this.comments[comment.matchId] || [];
    this.comments[comment.matchId] = [comment, ...list.filter((item) => item.id !== comment.id)];
  }

  private static applyBackendSnapshot(snapshot: {
    matches: Match[];
    ...
    this.remoteStats = snapshot.stats;
    this.currentUser = snapshot.user;
    this.userVotes = {};
    snapshot.user?.votedMatchIds.forEach((vote) => {
      this.userVotes[vote.matchId] = vote.creatorId;
    });
  }

  static async connectBackend(): Promise<boolean> {
    if (this.backendSyncPromise) return this.backendSyncPromise;
    this.backendSyncPromise = (async () => {
      try {
        const health = await fetch(`${BACKEND_URL}/api/health`, { credentials: 'include' });
  ...
  private static async refreshBackend() {
    ...
    try {
      ...
      // Keep the last known state visible when a short network interruption occurs.
    }
  }
  ...
  private static notify(persist = true) {
    if (persist && typeof window !== 'undefined') {
      try {
        localStorage.setItem(MATCHES_STORAGE_KEY, JSON.stringify(this.matches));
        localStorage.setItem(COMMENTS_STORAGE_KEY, JSON.stringify(this.comments));
        localStorage.setItem(VOTES_STORAGE_KEY, JSON.stringify(this.userVotes));
        localStorage.setItem(LIKES_STORAGE_KEY, JSON.stringify(this.userLikes));
      } catch (err) {
  ...
  // Count one local fan reaction per match and creator.
  static addLike(matchId: string, creatorId: string, count: number = 1): { success: boolean; likes1: number; likes2: number } {
    ...
    const likeKey = `${match.id}:${creatorId}`;
    if (this.userLikes[likeKey]) {
      return { success: false, likes1: match.likes1, likes2: match.likes2 };
    }
    ...
    this.userLikes[likeKey] = true;

    this.notify();
    if (this.backendAvailable) {
      void this.backendRequest<{ match: Match }>(`/api/matches/${encodeURIComponent(match.id)}/likes`, {
        method: 'POST',
        body: JSON.stringify({ creatorId }),
      }).then(({ match: savedMatch }) => {
        this.applyBackendMatch(savedMatch);
        this.notify(false);
      }).catch(() => this.refreshBackend());
    }
  ...
  static vote(matchId: string, creatorId: string): { success: boolean; message: string; match?: Match; deltaP?: number } {
    ...
    if (!isFirstVote) {
      return { success: false, message: 'You already voted in this battle.' };
    }
    ...
    if (this.backendAvailable) {
      void this.backendRequest<{ match: Match; user: UserProfile | null }>(`/api/matches/${encodeURIComponent(match.id)}/vote`, {
        method: 'POST',
        body: JSON.stringify({ creatorId }),
      }).then(({ match: savedMatch, user }) => {
        this.applyBackendMatch(savedMatch);
    ...
    const surgeMsg = isCreator1
      ? `+1 Vote for ${match.creator1.name}! Trend moved +${Math.abs(deltaP)}%`
      : `+1 Vote for ${match.creator2.name}! Trend moved +${Math.abs(deltaP)}%`;

    return {
      success: true,
      message: `Official Vote Registered! ${surgeMsg}`,
      match,
      deltaP,
    };
  }
  ...
  static addComment(matchId: string, authorName: string, content: string, allegianceCreatorId?: string): Comment {
    ...
    this.notify();
    if (this.backendAvailable) {
      void this.backendRequest<{ comment: Comment }>(`/api/matches/${encodeURIComponent(matchId)}/comments`, {
        method: 'POST',
        body: JSON.stringify({ authorName, content, allegianceCreatorId }),
      }).then(({ comment }) => {
        this.comments[matchId] = [comment, ...(this.comments[matchId] || []).filter((item) => item.id !== newComment.id)];
        this.notify(false);
      }).catch(() => this.refreshBackend());
    }
    return newComment;
  ...
  static likeComment(matchId: string, commentId: string) {
    ...
    if (list) {
      ...
      if (c) {
        ...
        this.notify();
        if (this.backendAvailable) {
          void this.backendRequest<{ comment: Comment }>(`/api/comments/${encodeURIComponent(commentId)}/likes`, {
            method: 'POST',
          }).then(({ comment }) => {
            this.applyBackendComment(comment);
            this.notify(false);
          }).catch(() => this.refreshBackend());
        }
      }
  ...
  static createMatch(matchData: Partial<Match>): Match {
    ...
    this.notify();
    if (this.backendAvailable) {
      void this.backendRequest<{ match: Match }>('/api/admin/matches', {
        method: 'POST',
        body: JSON.stringify(newMatch),
      }).then(({ match }) => {
        this.matches = [match, ...this.matches.filter((item) => item.id !== newMatch.id)];
        this.notify(false);
      }).catch(() => this.refreshBackend());
    }
    return newMatch;
  ...
  static endMatchManually(matchId: string, winnerId?: string) {
    ...
    if (match) {
      ...
      this.notify();
      if (this.backendAvailable) {
        void this.backendRequest<{ match: Match }>(`/api/admin/matches/${encodeURIComponent(matchId)}/end`, {
          method: 'POST',
          body: JSON.stringify({ winnerId: match.winnerId }),
        }).then(({ match: savedMatch }) => {
          this.applyBackendMatch(savedMatch);
          this.notify(false);
        }).catch(() => this.refreshBackend());
  ...
  static getStats(): SiteStats {
    if (this.remoteStats) return this.remoteStats;
  ...
  static async getLeaderboard() {
    if (this.backendAvailable) {
      try {
        const data = await this.backendRequest<{ leaderboard: Array<{ rank: number; name: string; votes: number; points: number; badge: string }> }>('/api/leaderboard');
        return data.leaderboard;
      } catch {
        // Fall back to the local account when the backend is briefly unavailable.
      }
    }
    return [];
  }
  ...
  static async createBackup() {
    if (!this.backendAvailable) throw new Error('Backup service is only available on the production backend.');
    return this.backendRequest<{ backup: string }>('/api/admin/backup', { method: 'POST' });
  }
  ...
  static async authenticate(name: string, email: string, password: string): Promise<{ user?: UserProfile; error?: string }> {
    if (!this.backendAvailable) return { user: this.login(name, email) };
    try {
      const data = await this.backendRequest<{ user: UserProfile }>('/api/auth/register', {
        method: 'POST',
        body: JSON.stringify({ name, email, password }),
      });
      this.currentUser = data.user;
      this.userVotes = {};
      data.user.votedMatchIds.forEach((vote) => { this.userVotes[vote.matchId] = vote.creatorId; });
      this.notify(false);
      return data;
    } catch (error: any) {
      if (error?.code !== 'EMAIL_EXISTS') return { error: error.message || 'Unable to create your account.' };
      try {
        const data = await this.backendRequest<{ user: UserProfile }>('/api/auth/login', {
          method: 'POST',
        ...
        return { error: loginError.message || 'Unable to sign in.' };
      }
    }
  }
  ...
  static async logoutBackend() {
    if (this.backendAvailable) {
      await this.backendRequest('/api/auth/logout', { method: 'POST' }).catch(() => undefined);
    }
    this.logout();
  }
}
