...
export const AdSensePlaceholder: React.FC<AdSensePlaceholderProps> = ({ slotType, className = '' }) => {
  ...
  const clientId = (import.meta.env.VITE_ADSENSE_CLIENT_ID as string | undefined) || window.__RUNTIME_CONFIG__?.adsenseClientId;
  const configuredSlot = window.__RUNTIME_CONFIG__?.adsenseSlots?.[slotType];
  const buildSlot = slotType === 'header-banner'
    ? import.meta.env.VITE_ADSENSE_SLOT_HEADER_BANNER
    : slotType === 'in-content'
      ? import.meta.env.VITE_ADSENSE_SLOT_IN_CONTENT
      : slotType === 'sidebar'
        ? import.meta.env.VITE_ADSENSE_SLOT_SIDEBAR
        : import.meta.env.VITE_ADSENSE_SLOT_FOOTER_BANNER;
  const slot = configuredSlot || buildSlot;
  ...
  useEffect(() => {
    ...
    try {
      ...
      // AdSense can be blocked by privacy tools without affecting the app.
    }
  }, [clientId, slot]);
        ...
        {clientId && slot ? (
          ...
          <>
            ...
            <span className="text-[10px] text-slate-600 mt-0.5">
              Ad slot ready. Add the AdSense client and slot IDs to activate ads.
            </span>
          </>
        )}
      </div>
