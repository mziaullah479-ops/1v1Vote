...
export const MatchArena: React.FC<MatchArenaProps> = ({
  ...
  // Unlimited likes handler - ANY user can like as many times as they want!
  ...
  const totalLikes = likes1 + likes2 || 1;
  const likesRatio1 = Math.round((likes1 / totalLikes) * 100);
  const likesRatio2 = 100 - likesRatio1;

  // Vote stats
  const totalVotes = match.votes1 + match.votes2 || 1;
  const votesRatio1 = ((match.votes1 / totalVotes) * 100).toFixed(1);
  const votesRatio2 = ((match.votes2 / totalVotes) * 100).toFixed(1);
    ...
    <div className="relative w-full max-w-6xl mx-auto py-2 px-2 sm:px-4">
      ...
      <div className="mb-4 bg-gradient-to-r from-sky-950/60 via-slate-900/80 to-amber-950/60 border border-sky-500/30 rounded-2xl p-2.5 sm:px-4 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-300">
        ...
        <div className="flex items-center gap-2 text-[11px] text-sky-400 font-semibold">
          ...
          <span>Unlimited Likes: Tap as many times as you like!</span>
        </div>
      </div>
          ...
          >
            ...
            <div className="mt-4 p-2.5 rounded-2xl bg-[#091428] border border-sky-500/20 flex items-center justify-between gap-2">
              ...
              <button
                ...
                title="Click as many times as you want!"
              >
            ...
            <div className="mt-4 flex flex-col items-center">
              ...
              >
                {isVotedC1 ? (
                  <>
                    ...
                    <span>Voted ✓ (Click for Power Boost)</span>
                  </>
                ) : (
                  <>
              ...
              <div className="mt-2.5 flex flex-wrap items-center justify-between gap-1.5 w-full text-[11px] text-slate-400 px-1">
                <div className="flex items-center gap-2">
                  <span>Votes: <strong className="text-white font-mono">{match.votes1.toLocaleString()}</strong></span>
                  {/* Visual Sentiment Meter (Blue to Orange Gradient Velocity) */}
                  <SentimentMeter match={match} creatorId={match.creator1.id} variant="inline" />
                </div>
                <button
                  onClick={() => onSelectCreatorDetail && onSelectCreatorDetail(match.creator1.id)}
            ...
            <div className="mt-4 flex flex-col items-center">
              ...
              <div className="mt-2.5 flex flex-wrap items-center justify-between gap-1.5 w-full text-[11px] text-slate-400 px-1">
                <div className="flex items-center gap-2">
                  ...
                  {/* Visual Sentiment Meter (Blue to Orange Gradient Velocity) */}
                  <SentimentMeter match={match} creatorId={match.creator2.id} variant="inline" />
                </div>
                <button
                  onClick={() => onSelectCreatorDetail && onSelectCreatorDetail(match.creator2.id)}
