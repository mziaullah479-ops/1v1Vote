...
import { X, Download, Share2, Copy, Check, Eye } from 'lucide-react';
...
export const ShareModal: React.FC<ShareModalProps> = ({ match, isOpen, onClose }) => {
  ...
  const handleCopyLink = () => {
    ...
    void MatchStore.recordShare(match.id);
    setCopied(true);
  ...
  const handleNativeShare = async () => {
    ...
    try {
      await navigator.share({
        title: `${match.creator1.name} vs ${match.creator2.name} - 1v1Vote`,
        text: `Vote in ${match.creator1.name} vs ${match.creator2.name} on 1v1Vote.`,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
      <div className="relative w-full max-w-2xl bg-[#090f1d] border border-slate-700/80 rounded-3xl p-6 sm:p-8 shadow-[0_0_50px_rgba(0,0,0,0.8)] overflow-hidden">
        ...
        <div className="flex items-center gap-4 text-[11px] text-slate-500 mb-4">
          <span className="flex items-center gap-1"><Eye className="w-3.5 h-3.5 text-sky-400" /> {(match.views || 0).toLocaleString()} views</span>
          <span className="flex items-center gap-1"><Share2 className="w-3.5 h-3.5 text-amber-400" /> {(match.shares || 0).toLocaleString()} shares</span>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mb-4">
          <a
            ...
            onClick={() => void MatchStore.recordShare(match.id)}
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl bg-blue-600/20 hover:bg-blue-600/30 border border-blue-500/50 text-blue-200 text-xs font-bold transition-all"
          >
