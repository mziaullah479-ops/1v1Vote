...
const quickQuestions = ['How does voting work?', 'How does the public signal market work?', 'Can paid promotion change votes?', 'Where do profiles come from?'];
...
function answerQuestion(question: string, profileCount: number) {
  ...
  if (/market|trend|growth|signal|chart|dashboard/.test(value)) return 'Each profile has a public-signal dashboard with 1-day, 1-week, 1-month, 1-year, and 5-year trend views. It uses public profile strength, visible reach where available, and separate site activity. It never creates votes and is not a financial price.';
  if (/vote|cooldown|daily|midnight|24/.test(value)) return 'Visitors can cast one organic vote for each active profile per calendar day. The next vote becomes available after midnight Asia/Karachi time. Votes are stored server-side and rankings use organic votes first, then shares, then name.';
...
export const AiGuideView: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([{ role: 'assistant', text: 'I can explain voting, public-source profiles, the public-signal market dashboard, and how sponsored promotion stays separate from organic votes.' }]);
